...
compile_slave() {
  ...
  if [[ "$EXPERIMENT_NAME" == "master_i3c_dma_official_rx_smartdma_wake_block_stream" ]]; then
    stream_block_bytes=$(extract_define_value "${RT595_EXTRA_MASTER_DEFINES:-}" I3C_STREAM_BLOCK_BYTES || true)
    if [[ -z "$stream_block_bytes" ]]; then
      stream_block_bytes=240
    fi
    defines+=(EXPERIMENT_SLAVE_STREAM_SOURCE=1 EXPERIMENT_SLAVE_STREAM_BLOCK_BYTES="$stream_block_bytes")
  fi
