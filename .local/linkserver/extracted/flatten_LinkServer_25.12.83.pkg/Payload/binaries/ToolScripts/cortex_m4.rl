...
# Built-in register support for a Cortex M4
...
set fld.TPIU [
    ...
    DEVID = [
        TRINCNT   = <0,5,[n]:{strf "%d trace inputs"<1_shl_n>!}>,
        ASYNCTRCLK= <5,1,choicemsg <"Async TRACECLKIN not supported",
                                    "Async TRACECLKIN supported">>, 
...
# Definition of registers in an ETM peripheral on a Coretex M4
...
# Definition of registers in an ITM peripheral on a Coretex M4
set dev.ITM[rd,wr]:{[
    # Cortex M3:
...
# System control space registers for a Cortex M4 - Core debug control regisers
...
set dev.SCS[rd,wr]:{[
    ...
    IEBR0      = <0xFB0,rd,rd,wr>,
    IEBR1      = <0xFB4,rd,rd,wr>,
    DEBR0      = <0xFB8,rd,rd,wr>,
    EEBR1      = <0xFBC,rd,rd,wr>,
    ]
}
...
# Data Watchpoint Trace registers for a Cortex-M4
set dev.DWT[rd,wr]:{[
    # Cortex M3:
    ...
    # Cortext M7:
    DWT_LAR       = <0x058,NULL,NULL,wr>,
    DWT_LSR       = <0x058,rd,rd,NULL>,
    ]
}
...
set local.fld_DWT_FUNCTION [
        ...
        EMITRANGE     = <5,1,endismsg>,
        CYCMATCH      = <6,1,bitmsg>, # DWT_FUNCITON0 only
        DATAVMATCH    = <7,1,bitmsg>,
        LNK1ENA       = <9,1,endismsg>,
        DATAVSIZE     = <19,2,choicemsg<"byte","halfword","word","<reserved>">>,
        DATAVADDR0    = <12,4,intmsg "comparitor %d">,
        DATAVADDR1    = <16,4,intmsg "comparitor %d">,
        MATCHED       = <24,1,bitmsg>,
]
...
# Flash Breakpoint regisers for Cortex-M4
set dev.FBP[rd,wr]:{[
    # Cortex M3:
