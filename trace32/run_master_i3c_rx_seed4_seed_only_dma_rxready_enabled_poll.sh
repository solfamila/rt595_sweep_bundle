...
master_axf="$bundle_root/master_i3c_rx_seed4_seed_only_dma_rxready_enabled_poll/_build/master/evkmimxrt595_ezhb.axf"

if [[ ! -f "$master_axf" ]]; then
  echo "missing master ELF: $master_axf" >&2
  echo "Build it first with: RT595_MASTER_RUN_MODE=none ./run_experiment.sh master_i3c_rx_seed4_seed_only_dma_rxready_enabled_poll" >&2
  exit 1
fi
