...
/* DEBUG(i3c_errata_052041): instrumentation globals captured around the
 * SDK ERR052123 RX-loop workaround. */
volatile uint32_t g_i3c_dbg_rxloop_count = 0U;
volatile uint32_t g_i3c_dbg_rxloop_dataSize = 0U;
volatile uint32_t g_i3c_dbg_rxloop_transDataSize = 0U;
volatile uint32_t g_i3c_dbg_rxloop_before_mdatactrl = 0U;
volatile uint32_t g_i3c_dbg_rxloop_after_mdatactrl = 0U;
volatile uint32_t g_i3c_dbg_rxloop_before_mstatus = 0U;
volatile uint32_t g_i3c_dbg_rxloop_before_merrwarn = 0U;
volatile uint32_t g_i3c_dbg_rxloop_first_before_mdatactrl = 0U;
volatile uint32_t g_i3c_dbg_rxloop_first_before_mstatus = 0U;
volatile uint32_t g_i3c_dbg_rxloop_first_before_merrwarn = 0U;
volatile uint32_t g_i3c_dbg_rxloop_first_dataSize = 0U;

/* DEBUG(i3c_errata_052041): captured at top of I3C_MasterTransferDMA, which is
 * the entry to a fresh transfer (CCC or bulk). Useful to see what state the
 * previous transfer left in MDATACTRL/MERRWARN. */
volatile uint32_t g_i3c_dbg_xfer_count = 0U;
...
enum _i3c_dma_flag_constants {
  ...
  kMasterClearFlags = kI3C_MasterSlaveStartFlag | kI3C_MasterControlDoneFlag |
                      kI3C_MasterCompleteFlag | kI3C_MasterArbitrationWonFlag |
                      kI3C_MasterSlave2MasterFlag | kI3C_MasterErrorFlag,

  /*! IRQ sources enabled by the non-blocking transactional API. */
  kMasterDMAIrqFlags = kI3C_MasterSlaveStartFlag | kI3C_MasterControlDoneFlag |
                       kI3C_MasterCompleteFlag | kI3C_MasterArbitrationWonFlag |
                       kI3C_MasterErrorFlag | kI3C_MasterSlave2MasterFlag,

  /*! Errors to check for. */
  kMasterErrorFlags = kI3C_MasterErrorNackFlag |
                      kI3C_MasterErrorWriteAbortFlag |
#if !defined(FSL_FEATURE_I3C_HAS_NO_MERRWARN_TERM) || \
    (!FSL_FEATURE_I3C_HAS_NO_MERRWARN_TERM)
                      kI3C_MasterErrorTermFlag |
#endif
                      kI3C_MasterErrorParityFlag | kI3C_MasterErrorCrcFlag |
                      kI3C_MasterErrorReadFlag | kI3C_MasterErrorWriteFlag |
                      kI3C_MasterErrorMsgFlag | kI3C_MasterErrorInvalidReqFlag |
                      kI3C_MasterErrorTimeoutFlag,
  /*! All flags which are cleared by the driver upon starting a transfer. */
  kSlaveClearFlags =
      kI3C_SlaveBusStartFlag | kI3C_SlaveMatchedFlag | kI3C_SlaveBusStopFlag,
  ...
  kSlaveDMAIrqFlags = kI3C_SlaveBusStartFlag | kI3C_SlaveMatchedFlag |
                      ...
                      kI3C_SlaveReceivedCCCFlag | kI3C_SlaveErrorFlag |
                      kI3C_SlaveHDRCommandMatchFlag | kI3C_SlaveCCCHandledFlag |
                      kI3C_SlaveEventSentFlag,

  /*! Errors to check for. */
  kSlaveErrorFlags = kI3C_SlaveErrorOverrunFlag | kI3C_SlaveErrorUnderrunFlag |
                     kI3C_SlaveErrorUnderrunNakFlag | kI3C_SlaveErrorTermFlag |
                     kI3C_SlaveErrorInvalidStartFlag |
                     kI3C_SlaveErrorSdrParityFlag |
                     kI3C_SlaveErrorHdrParityFlag | kI3C_SlaveErrorHdrCRCFlag |
                     kI3C_SlaveErrorS0S1Flag | kI3C_SlaveErrorOverreadFlag |
...
#if defined(I3C_ERRATA_052123_USE_DMA_CHAIN) && \
    (I3C_ERRATA_052123_USE_DMA_CHAIN)
/* Pre-allocated chain descriptor table for RX to avoid runtime allocation. */
SDK_ALIGN(static dma_descriptor_t s_dma_table[ARRAY_SIZE(kI3cBases)]
                                             [I3C_DMA_CHAIN_COUNT_MAX],
          FSL_FEATURE_DMA_LINK_DESCRIPTOR_ALIGN_SIZE);
#else
/*! @brief DMA linked transfer descriptor. */
SDK_ALIGN(static dma_descriptor_t s_dma_table[ARRAY_SIZE(kI3cBases)][2],
          FSL_FEATURE_DMA_LINK_DESCRIPTOR_ALIGN_SIZE);
#endif
...
static void I3C_MasterSetDMARxLoop(i3c_master_dma_handle_t* handle) {
  ...
  /* DEBUG(i3c_errata_052041): snapshot HW state at entry to the workaround,
   * BEFORE watermarks are (re)configured. */
  g_i3c_dbg_rxloop_dataSize = handle->transfer.dataSize;
  g_i3c_dbg_rxloop_transDataSize = handle->transDataSize;
  g_i3c_dbg_rxloop_before_mdatactrl = handle->base->MDATACTRL;
  g_i3c_dbg_rxloop_before_mstatus = handle->base->MSTATUS;
  g_i3c_dbg_rxloop_before_merrwarn = handle->base->MERRWARN;
  if (g_i3c_dbg_rxloop_count == 0U) {
    g_i3c_dbg_rxloop_first_before_mdatactrl = g_i3c_dbg_rxloop_before_mdatactrl;
    g_i3c_dbg_rxloop_first_before_mstatus = g_i3c_dbg_rxloop_before_mstatus;
    g_i3c_dbg_rxloop_first_before_merrwarn = g_i3c_dbg_rxloop_before_merrwarn;
    g_i3c_dbg_rxloop_first_dataSize = g_i3c_dbg_rxloop_dataSize;
  }
  g_i3c_dbg_rxloop_count++;
#if defined(I3C_ERRATA_052123_USE_DMA_CHAIN) && \
    (I3C_ERRATA_052123_USE_DMA_CHAIN)
  ...
  rxTriggerLevel =
      (handle->transfer.dataSize >= 6U)
          ? kI3C_RxTriggerUntilThreeQuarterOrMore
          : (i3c_rx_trigger_level_t)(handle->transfer.dataSize / 2U);
  I3C_MasterSetWatermarks(
      handle->base, kI3C_TxTriggerOnEmpty, rxTriggerLevel, false, false);
  ...
  for (uint32_t i = 0U; i < descCount; i++) {
    ...
    /* Need to adjust the watermark before last DMA descriptor, so enable
     * interrupts for the second-to-last DMA descriptor. */
    intr = (leftDataSize >= 12U) ? false : true;

    DMA_SetupDescriptor(
        &s_dma_table[instance][i],
        DMA_CHANNEL_XFER((i < (descCount - 1U)),
                         false,
                         intr,
                         false,
                         sizeof(uint8_t),
    ...
    /* Let DMA interrupt know that there's left data to copy with watermark
     * update. */
    if (unitDataLen == 6U) {
  ...
  if (leftDataSize >= 6U) {
    I3C_MasterSetWatermarks(handle->base,
  ...
  DMA_SetupDescriptor(&s_dma_table[instance][0],
                      DMA_CHANNEL_XFER(false,
                                       ...
                                       unitDataLen),
                      (void*)(uint32_t*)(uint32_t)&handle->base->MRDATAB,
                      (uint8_t*)handle->transfer.data + handle->transDataSize,
                      NULL);
  handle->transDataSize += unitDataLen;
#endif
  DMA_SetChannelConfig(
      handle->rxDmaHandle->base, handle->rxDmaHandle->channel, NULL, true);
...
static void I3C_MasterTransferDMACallbackRx(dma_handle_t* dmaHandle,
                                            void* param,
                                            bool transferDone,
                                            uint32_t tcds) {
  ...
  if (transferDone) {
    ...
    while (i3cHandle->transDataSize < i3cHandle->transfer.dataSize) {
      uint32_t rxCount =
          (i3cHandle->base->MDATACTRL & I3C_MDATACTRL_RXCOUNT_MASK) >>
          I3C_MDATACTRL_RXCOUNT_SHIFT;
      ...
      while ((rxCount != 0U) &&
             (i3cHandle->transDataSize < i3cHandle->transfer.dataSize)) {
        ((uint8_t*)i3cHandle->transfer.data)[i3cHandle->transDataSize++] =
            (uint8_t)(i3cHandle->base->MRDATAB & I3C_MRDATAB_VALUE_MASK);
        rxCount--;
      }
    }

    uint32_t leftDataSize =
        i3cHandle->transfer.dataSize - i3cHandle->transDataSize;
    if (leftDataSize > 0U) {
#if (defined(I3C_ERRATA_052123_USE_DMA_CHAIN) && \
     (I3C_ERRATA_052123_USE_DMA_CHAIN))
      i3c_rx_trigger_level_t rxTriggerLevel;
      rxTriggerLevel = (leftDataSize >= 6U)
                           ? kI3C_RxTriggerUntilThreeQuarterOrMore
                           : (i3c_rx_trigger_level_t)(leftDataSize / 2U);
      I3C_MasterSetWatermarks(
          i3cHandle->base, kI3C_TxTriggerOnEmpty, rxTriggerLevel, false, false);
      i3cHandle->transDataSize += (1U << (leftDataSize / 2U));
#else
      I3C_MasterSetDMARxLoop(i3cHandle);
#endif
    } else {
#endif
      /* If target send back the data logner as expect and controller can't get
         COMPLETE, stop the transfer here. If bus transfer is done normally,
         this function will do nothing. */
      if ((i3cHandle->state != (uint8_t)kCtrlDoneState) &&
          (0UL ==
           (i3cHandle->base->MINTMASKED & (uint32_t)kI3C_MasterCompleteFlag))) {
...
#if defined(FSL_FEATURE_I3C_HAS_ERRATA_052123) && \
    (FSL_FEATURE_I3C_HAS_ERRATA_052123)
static void I3C_MasterSetDMATxLoop(I3C_Type* base,
  ...
  uint32_t byteAddr =
      isSubAddr ? (uint32_t)&base->MWDATAB : (uint32_t)&base->MWDATABE;
  uint32_t halfWordAddr =
      isSubAddr ? (uint32_t)&base->MWDATAH : (uint32_t)&base->MWDATAHE;
#if defined(I3C_ERRATA_052123_USE_DMA_CHAIN) && \
    (I3C_ERRATA_052123_USE_DMA_CHAIN)
  uint8_t* pData = data;
    ...
    if (leftSize == 1U) {
      DMA_SetupDescriptor(&s_dma_table[instance][j],
                          DMA_CHANNEL_XFER(false,
                                           ...
                                           kDMA_AddressInterleave0xWidth,
                                           1U),
                          pData + handle->transDataSize,
                          (void*)(uint32_t*)byteAddr,
                          NULL);
    } else if (leftSize == 2U) {
      DMA_SetupDescriptor(&s_dma_table[instance][j],
                          DMA_CHANNEL_XFER(false,
                                           ...
                                           2U),
                          pData + handle->transDataSize,
                          (void*)(uint32_t*)halfWordAddr,
                          NULL);
    } else if (leftSize <= 8U) {
      (void)memset(
          &handle->workaroundBuff[j][0], 0, sizeof(handle->workaroundBuff[j]));
      for (uint32_t i = 0; i < leftSize - 1U; i += 2U) {
        (void)memcpy((void*)&handle->workaroundBuff[j][i * 2U],
                     (void*)(pData + handle->transDataSize + i),
                     2);
      }

      DMA_SetupDescriptor(&s_dma_table[instance][j],
                          DMA_CHANNEL_XFER(true,
                                           false,
                                           false,
                                           false,
                                           WIDTH_4_BYTE,
                                           ...
                                           kDMA_AddressInterleave0xWidth,
                                           ((leftSize - 1U) / 2U) * 4U),
                          &handle->workaroundBuff[j][0],
                          (void*)(uint8_t*)(uint32_t)&base->MWDATAH,
                          &s_dma_table[instance][j + 1U]);

      if ((leftSize % 2U) != 0U) {
        (void)memcpy(
            (void*)&handle->workaroundBuff[j][((leftSize - 1U) / 2U) * 4U],
            (void*)(pData + handle->transDataSize + leftSize - 1U),
            1);
        DMA_SetupDescriptor(
            &s_dma_table[instance][j + 1U],
            DMA_CHANNEL_XFER(false,
      ...
      DMA_SetupDescriptor(&s_dma_table[instance][j],
                          DMA_CHANNEL_XFER(true,
                                           ...
                                           kDMA_AddressInterleave0xWidth,
                                           16U),
                          (void*)&handle->workaroundBuff[j][0],
                          (void*)(uint32_t*)(uint32_t)&base->MWDATAH,
                          &s_dma_table[instance][j + 1U]);
    }
  ...
  if (dataSize == 1U) {
    DMA_SetupDescriptor(&s_dma_table[instance][0],
                        DMA_CHANNEL_XFER(false,
                                         ...
                                         kDMA_AddressInterleave0xWidth,
                                         1U),
                        (uint8_t*)data + handle->transDataSize,
                        (void*)(uint32_t*)byteAddr,
                        NULL);
  } else if (dataSize == 2U) {
    DMA_SetupDescriptor(&s_dma_table[instance][0],
    ...
    for (uint32_t i = 0; i < dataSize - 1U; i += 2U) {
      (void)memcpy((void*)&handle->workaroundBuff[i * 2U],
                   (void*)((uint8_t*)data + handle->transDataSize + i),
                   2);
    }
    ...
    DMA_SetupDescriptor(&s_dma_table[instance][0],
                        DMA_CHANNEL_XFER(true,
                                         ...
                                         kDMA_AddressInterleave0xWidth,
                                         ((dataSize - 1U) / 2U) * 4U),
                        &handle->workaroundBuff[0],
                        (void*)(uint8_t*)(uint32_t)&base->MWDATAH,
                        &s_dma_table[instance][1]);

    if ((dataSize % 2U) != 0U) {
      (void)memcpy(
          (void*)&handle->workaroundBuff[((dataSize - 1U) / 2U) * 4U],
          (void*)((uint8_t*)data + handle->transDataSize + dataSize - 1U),
          1);
...
#endif
  DMA_SetChannelConfig(
      handle->txDmaHandle->base, handle->txDmaHandle->channel, NULL, true);
  DMA_SubmitChannelDescriptor(handle->txDmaHandle, &s_dma_table[instance][0]);
  DMA_StartTransfer(handle->txDmaHandle);
}
#endif

static void I3C_MasterTransferDMACallbackTx(dma_handle_t* dmaHandle,
                                            void* param,
                                            bool transferDone,
                                            uint32_t tcds) {
  i3c_master_dma_handle_t* i3cHandle = (i3c_master_dma_handle_t*)param;
  ...
  g_i3c_dbg_dma_callback_tx_mdatactrl = i3cHandle->base->MDATACTRL;
    ...
    (FSL_FEATURE_I3C_HAS_ERRATA_052123)
    uint32_t leftBytes =
        (i3cHandle->transfer.direction == kI3C_Read)
            ? 0U
            : i3cHandle->transferCount - i3cHandle->transDataSize;
    if (leftBytes == 0U) {
#endif
...
static void I3C_MasterSetTxDMA(I3C_Type* base,
                               i3c_master_dma_handle_t* handle,
...
#if defined(FSL_FEATURE_I3C_HAS_NO_MASTER_DMA_WDATA_REG) && \
    (FSL_FEATURE_I3C_HAS_NO_MASTER_DMA_WDATA_REG)
    address = (uint32_t)&base->MWDATAB;
#else
    address = (uint32_t)&base->MWDATAB1;
#endif
    DMA_PrepareChannelTransfer(txChannelConfig,
                               data,
                               (void*)(uint32_t*)address,
                               DMA_CHANNEL_XFER(false,
                                                false,
                                                false,
                                                false,
...
static void I3C_MasterRunDMATransfer(I3C_Type* base,
                                     i3c_master_dma_handle_t* handle,
  ...
  if (direction == kI3C_Write) {
    ...
    if (handle->subaddressCount != 0U) {
      if (dataSize != 0U) {
        I3C_MasterSetDMATxLoop(base,
                               handle,
                               &handle->subaddressBuffer[0],
                               handle->subaddressCount,
                               true);
      } else {
    ...
    (FSL_FEATURE_I3C_HAS_NO_MASTER_DMA_WDATA_REG)
      ...
      data = (handle->subaddressCount != 0U) ? (&handle->subaddressBuffer[0])
                                             : handle->transfer.data;
      dataSize = (handle->subaddressCount != 0U) ? handle->subaddressCount
...
static status_t I3C_MasterInitTransferStateMachineDMA(
    I3C_Type* base, i3c_master_dma_handle_t* handle) {
  assert(handle->transfer.subaddressSize <=
         sizeof(handle->transfer.subaddress));
  ...
  if (xfer->busType != kI3C_TypeI3CDdr) {
    direction = (0UL != xfer->subaddressSize) ? (i3c_direction_t)kI3C_Write
                                              : xfer->direction;
  }

  /* For combination of write address and read data, need to do ReSTART after
   * write. */
  ...
  if (0U != (xfer->flags & (uint32_t)kI3C_TransferNoStartFlag)) {
    ...
    if (0U != (xfer->flags & (uint32_t)kI3C_TransferRepeatedStartFlag)) {
      result = I3C_MasterRepeatedStart(
          base, xfer->busType, xfer->slaveAddress, direction);
    } else {
      result =
          I3C_MasterStart(base, xfer->busType, xfer->slaveAddress, direction);
    }
  }
...
static status_t I3C_MasterRunTransferStateMachineDMA(
    I3C_Type* base, i3c_master_dma_handle_t* handle, bool* isDone) {
  status_t result = kStatus_Success;
  ...
  *isDone = false;
  ...
  if ((masterState == kI3C_MasterStateIbiRcv) ||
      (masterState == kI3C_MasterStateIbiAck)) {
  ...
  if (handle->state == (uint8_t)kIBIWonState) {
    /* Get Rx fifo counts. */
    rxCount = (base->MDATACTRL & I3C_MDATACTRL_RXCOUNT_MASK) >>
              I3C_MDATACTRL_RXCOUNT_SHIFT;
  }
  ...
  while (!state_complete) {
      ...
      case (uint8_t)kIBIWonState:
        if (masterState == kI3C_MasterStateIbiAck) {
          ...
          if (handle->callback.ibiCallback != NULL) {
            handle->callback.ibiCallback(
                base, handle, handle->ibiType, kI3C_IbiAckNackPending);
          } else {
        ...
        if (0UL != rxCount) {
          if ((handle->ibiBuff == NULL) &&
              (handle->callback.ibiCallback != NULL)) {
            handle->callback.ibiCallback(
                base, handle, kI3C_IbiNormal, kI3C_IbiDataBuffNeed);
          }
    ...
    (FSL_FEATURE_I3C_HAS_ERRATA_052123)
          ...
          if (handle->transfer.dataSize < 256U) {
            result = I3C_MasterRepeatedStartWithRxSize(
                base,
                xfer->busType,
                xfer->slaveAddress,
                kI3C_Read,
            ...
            result = I3C_MasterRepeatedStart(base,
                                             handle->transfer.busType,
    ...
    (FSL_FEATURE_I3C_HAS_ERRATA_052123)
      case (uint8_t)kTransferDataState:
        if (0UL != (status & (uint32_t)kI3C_MasterTxReadyFlag)) {
          uint32_t leftBytes =
              (handle->transfer.direction == kI3C_Read)
                  ? 0U
                  : handle->transferCount - handle->transDataSize;
          if (leftBytes > 0U) {
            I3C_MasterSetDMATxLoop(
                base, handle, handle->transfer.data, leftBytes, false);
            state_complete = true;
          } else {
            I3C_MasterDisableInterrupts(base, (uint32_t)kI3C_MasterTxReadyFlag);
            handle->state = (uint8_t)kWaitForCompletionState;
...
void I3C_MasterTransferCreateHandleDMA(
    I3C_Type* base,
    i3c_master_dma_handle_t* handle,
 ...
 * retval #kStatus_I3C_Busy Either another master is currently utilizing the
 * bus, or a non-blocking transaction is already in progress.
 */
status_t I3C_MasterTransferDMA(I3C_Type* base,
                               i3c_master_dma_handle_t* handle,
  ...
  g_i3c_dbg_xfer_last_direction = (uint32_t)transfer->direction;
  g_i3c_dbg_xfer_last_dataSize = (uint32_t)transfer->dataSize;
  g_i3c_dbg_xfer_last_subaddrSize = (uint32_t)transfer->subaddressSize;
  g_i3c_dbg_xfer_count++;
  ...
  checkDdrState = (transfer->busType == kI3C_TypeI3CDdr)
                      ? (masterState != kI3C_MasterStateDdr)
                      : true;
  if ((masterState != kI3C_MasterStateIdle) &&
      (masterState != kI3C_MasterStateNormAct) && checkDdrState) {
    return kStatus_I3C_Busy;
  }
  ...
  I3C_MasterDisableInterrupts(
      base,
      ((uint32_t)kMasterDMAIrqFlags | (uint32_t)kI3C_MasterRxReadyFlag |
  ...
  I3C_MasterEnableInterrupts(base,
                             (uint32_t)kMasterDMAIrqFlags
#if defined(FSL_FEATURE_I3C_HAS_ERRATA_052123) && \
    (FSL_FEATURE_I3C_HAS_ERRATA_052123)
                                 | (uint32_t)kI3C_MasterTxReadyFlag
#endif
...
void I3C_MasterTransferDMAHandleIRQ(I3C_Type* base, void* i3cHandle) {
  ...
  if (isDone || (result != kStatus_Success)) {
    ...
    if ((result == kStatus_I3C_IBIWon) &&
        (handle->callback.ibiCallback != NULL)) {
      handle->callback.ibiCallback(
          base, handle, handle->ibiType, kI3C_IbiReady);
 ...
 * transaction.
 */
status_t I3C_MasterTransferGetCountDMA(I3C_Type* base,
                                       i3c_master_dma_handle_t* handle,
                                       size_t* count) {
  assert(handle != NULL);
  ...
  /* There is no necessity to disable interrupts as we read a single integer
   * value */
  i3c_direction_t dir = handle->transfer.direction;

  if (dir == kI3C_Read) {
    *count = handle->transferCount -
             DMA_GetRemainingBytes(handle->rxDmaHandle->base,
                                   handle->rxDmaHandle->channel);
  } else {
    *count = handle->transferCount -
             DMA_GetRemainingBytes(handle->txDmaHandle->base,
                                   handle->txDmaHandle->channel);
  }
 ...
 * param handle pointer to i2c_master_dma_handle_t structure
 */
void I3C_MasterTransferAbortDMA(I3C_Type* base,
                                i3c_master_dma_handle_t* handle) {
  if (handle->state != (uint8_t)kIdleState) {
...
static void I3C_SlaveTransferDMACallback(dma_handle_t* dmaHandle,
                                         void* param,
                                         bool transferDone,
                                         uint32_t tcds) {
 ...
 *
 * For devices where the I3C send and receive DMA requests are OR'd together,
 * the @a txDmaHandle parameter is ignored and may be set to NULL.
 *
 * param base The I3C peripheral base address.
 * param handle Pointer to the I3C slave driver handle.
 * param callback User provided pointer to the asynchronous callback function.
 * param userData User provided pointer to the application callback data.
 * param rxDmaHandle Handle for the DMA receive channel. Created by the user
 * prior to calling this function. param txDmaHandle Handle for the DMA transmit
 * channel. Created by the user prior to calling this function.
 */
...
static void I3C_SlavePrepareTxDMA(I3C_Type* base,
                                  i3c_slave_dma_handle_t* handle) {
  ...
  if (xfer->txDataSize == 1U) {
    ...
    DMA_SetupDescriptor(&(s_dma_table[instance][0]),
                        DMA_CHANNEL_XFER(false,
                                         ...
                                         kDMA_AddressInterleave0xWidth,
                                         1U),
                        xfer->txData + xfer->txDataSize - 1U,
                        txFifoBase,
                        NULL);
  }
  (void)DMA_SubmitChannelTransfer(handle->txDmaHandle, &txChannelConfig);
...
static void I3C_SlavePrepareRxDMA(I3C_Type* base,
                                  i3c_slave_dma_handle_t* handle) {
  uint32_t* rxFifoBase = (uint32_t*)(uint32_t)&base->SRDATAB;
  i3c_slave_dma_transfer_t* xfer = &handle->transfer;
  dma_channel_config_t rxChannelConfig;

  DMA_PrepareChannelTransfer(&rxChannelConfig,
                             (void*)rxFifoBase,
                             xfer->rxData,
                             DMA_CHANNEL_XFER(false,
                                              true,
 ...
 *
 * The API will do DMA configuration according to the input transfer descriptor,
 * and the data will be transferred when there's bus master requesting transfer
 * from/to this slave. So the timing of call to this API need be aligned with
 * master application to ensure the transfer is executed as expected. Callback
 * specified when the @a handle was created is invoked when the transaction has
 * completed.
 *
 * param base The I3C peripheral base address.
 * param handle Pointer to the I3C slave driver handle.
 * param transfer The pointer to the transfer descriptor.
 * param eventMask Bit mask formed by OR'ing together
 * #i3c_slave_transfer_event_t enumerators to specify which events to send to
 * the callback. The transmit and receive events is not allowed to be enabled.
 * retval kStatus_Success The transaction was started successfully.
 * retval #kStatus_I3C_Busy Either another master is currently utilizing the
 * bus, or another DMA transaction is already in progress.
 */
...
void I3C_SlaveTransferDMAHandleIRQ(I3C_Type* base, void* i3cHandle) {
  ...
  if (0UL != (errFlags & (uint32_t)kSlaveErrorFlags)) {
    ...
    if ((0UL != (handle->eventMask & (uint32_t)kI3C_SlaveCompletionEvent)) &&
  ...
  if (0UL != (flags & (uint32_t)kI3C_SlaveEventSentFlag)) {
    ...
    if ((0UL != (handle->eventMask & xfer->event)) &&
        (NULL != handle->callback)) {
      handle->callback(base, xfer, handle->userData);
    }
  }
  ...
  if (0UL != (flags & (uint32_t)kI3C_SlaveMatchedFlag)) {
    ...
    handle->isBusy = true;
    if ((0UL != (handle->eventMask & (uint32_t)kI3C_SlaveAddressMatchEvent)) &&
        (NULL != handle->callback)) {
      handle->callback(base, xfer, handle->userData);
    }
  }
